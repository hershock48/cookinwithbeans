...
function memoryBag(): MemoryBag {
  const g = globalThis as unknown as { __beansOrdering?: MemoryBag };
  if (!g.__beansOrdering) {
    g.__beansOrdering = {
      orders: new Map(),
      state: { ...DEFAULT_STATE },
      ticket: 0,
...
async function pgPool(): Promise<Pool> {
  ...
  await g.__beansPgReady;
  return g.__beansPgPool;
}
