// The orderable menu: database-backed and staff-editable. Ported from the
// Copper AC build; the seed here is the TRUCK'S WINDOW BOARD (the six-step
// model from src/content/site.ts) rather than the raw Toast harvest, because
// the board is the better-configured menu: tortilla choice, cheese add-ons,
// real salsa names. Toast's page carries the same prices with sloppier
// modifiers. Extras the board does not sell are seeded hidden until the
// owner confirms them.
//
...
function cacheBag(): { cache: Cache | null } {
  const g = globalThis as unknown as { __beansMenuCache?: { cache: Cache | null } };
  g.__beansMenuCache ??= { cache: null };
  return g.__beansMenuCache;
}
