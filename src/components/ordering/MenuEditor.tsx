...
const inputCls =
  "w-full rounded-sm border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-100 outline-none focus:border-lime-300";
const smallBtn =
  "rounded-sm border border-neutral-700 px-3 py-1.5 text-xs text-neutral-400 transition-colors hover:border-lime-300";
...
export default function MenuEditor() {
  ...
  if (!doc) {
    return <p className="py-10 text-center text-sm text-neutral-400/70">{error || "Loading the menu..."}</p>;
  }

  return (
    <div>
      {/* Save rail: sticky so a long menu never hides the way out. */}
      <div className="sticky top-0 z-30 -mx-1 mb-6 flex flex-wrap items-center gap-3 border-b border-neutral-700 bg-neutral-950/95 px-1 py-3 backdrop-blur">
        <button
          type="button"
          onClick={save}
          disabled={!dirty || saving}
          className="font-display rounded-sm bg-lime-400 px-6 py-3 text-sm uppercase tracking-widest text-neutral-950 transition-colors hover:bg-lime-400-light disabled:cursor-not-allowed disabled:opacity-40"
        >
          {saving ? "Saving" : "Save menu"}
        </button>
        {dirty && <span className="text-sm text-lime-400">Unsaved changes</span>}
      ...
      {doc.map((section, si) => (
        <section key={si} className="mb-8 rounded-sm border border-neutral-700 bg-neutral-900 p-4">
          <div className="mb-3 flex flex-wrap items-center gap-3">
            <input
              value={section.name}
              onChange={(e) => mutate((d) => { d[si].name = e.target.value; })}
              aria-label="Section name"
              className="font-display max-w-xs rounded-sm border border-transparent bg-transparent px-2 py-1 text-base uppercase tracking-widest text-lime-400 outline-none focus:border-lime-300"
            />
    ...
    <li className="py-3">
      <div className="flex items-center justify-between gap-3">
        <button type="button" onClick={onToggle} className="min-w-0 flex-1 text-left" aria-expanded={open}>
          <span className={`text-sm ${item.hidden ? "text-neutral-400/50 line-through" : "text-neutral-100"}`}>
            {item.name || "(unnamed)"}
          </span>
          {item.hidden && (
            <span className="font-display ml-2 text-[10px] uppercase tracking-widest text-neutral-400/60">hidden</span>
          )}
        </button>
        <span className="text-sm text-neutral-400 tabular-nums">${toDollars(item.priceCents)}</span>
      ...
      {open && (
        <div className="mt-3 space-y-3 rounded-sm border border-neutral-700 bg-neutral-950 p-4">
          ...
          <label className="block text-xs text-neutral-400">
            Photo URL <span className="text-neutral-400/60">(blank for no photo; direct upload is coming)</span>
            <input
              value={item.image ?? ""}
          ...
          <div className="flex flex-wrap items-center gap-3 border-t border-neutral-700 pt-3">
            <label className="flex items-center gap-2 text-xs text-neutral-400">
              <input
                type="checkbox"
            ...
            <button
              ...
              className={`ml-auto rounded-sm border px-3 py-1.5 text-xs transition-colors ${
                armDelete ? "border-[#d9736b] bg-[#d9736b] text-neutral-950" : "border-neutral-700 text-neutral-400/70 hover:border-[#d9736b] hover:text-[#d9736b]"
              }`}
            >
...
function OptionGroups({
  ...
  return (
    <div className="space-y-3">
      <p className="font-display text-[10px] uppercase tracking-widest text-lime-400">Options</p>
      {item.groups.map((group, gi) => (
        <div key={gi} className="rounded-sm border border-neutral-700 p-3">
